...
class LEDStripSingleColor(LEDStripControlRequest):
  single_hex_color: str = Field(None, regex="^#[0-9a-fA-F]{6}$", description="hex_color should be in the format #0055FF")

class LEDStripSimplePattern(LEDStripControlRequest):
  pattern: List[tuple] = Field(None,description="An ordered list of RGB or RGBA tuples containing a custom light pattern")
  number_of_sequences: Optional[int] = Field(2,description="How many times should the pattern repeat") 
...
async def set_strip_power(strip_id: int, power: bool, db: Session = Depends(get_db)):
  ...
  dots = dotstar.DotStar(board.SCK, board.MOSI, strip.led_count, brightness=strip.default_brightness)
...
async def set_strip_color(strip_id: int, settings:LEDStripSingleColor, db: Session = Depends(get_db)):
  ...
  dots = dotstar.DotStar(board.SCK, board.MOSI, strip.led_count, brightness=brightness)
  rgb = webcolors.hex_to_rgb(strip.single_color_hex)
  dots.fill(rgb)
